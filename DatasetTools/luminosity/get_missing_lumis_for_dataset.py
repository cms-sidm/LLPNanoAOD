...
# get input path
datasets = sys.argv[1]
parent_dataset = sys.argv[2]
golden_json_path = sys.argv[3]
...
for dataset in datasets:
    ...
    if not processed_lumis:
        processed_lumis = dataset_lumis
    else:
        merge_lumis(processed_lumis, dataset_lumis)

parent_dataset_string = parent_dataset.replace('/', '_')
inputLumis = './output_LLPminiAOD_dataset_lumi/'+parent_dataset_string+'.txt'
command = 'dasgoclient --query="run,lumi dataset=' + parent_dataset + ' instance=prod/global" > ' + inputLumis
print(command)
...
output_file = './output_LLPminiAOD_dataset_lumi/'+parent_dataset_string+'_missing_lumis.txt'
